...
function Forminput() {
  ...
  useEffect(() => {
    if((locate.length === 0) || (service.length === 0) || (listProfesi === undefined) ) {
    ...
    <div>
      ...
      <div className="container-forminput">
       ...
       <form onSubmit={handleSubmit} >
        ...
        <input  type="text" name="numberPhone" value={profile.numberPhone} onChange={handleChange} />
        <label htmlFor="">No Hp</label><br />
        <input  type="number" name="price" value={profile.price} onChange={handleChange} />
        <label htmlFor="">Harga per Jam</label><br />
       <input type="text" name="description" value={profile.description} onChange={handleChange} />
       <label htmlFor="">Deskripsi</label><br />
       <input  type="date" name="endDateAvailable" value={profile.endDateAvailable} onChange={handleChange} />
       <label htmlFor="">Tanggal Akhir</label><br />
       <input  type="date" name="timeAvailable" value={profile.timeAvailable} onChange={handleChange} />
       <label htmlFor="">Waktu</label><br />
       <select name="locationId" value={profile.locationId} onChange={handleChange}>
         {locate.length !== 0 ? (
           locate.map((item, index) => (
              <option key={index} value={item._id } >{item.nameCity}</option>
           ))
         ) : (
          <p>Loading</p>
         ) }
       </select>
       <label htmlFor="">Lokasi</label><br />
       <select name="serviceId" value={profile.serviceId} onChange={handleChange}>
         {service.length !== 0 ? (
           service.map((item, index) => (
              <option key={index} value={item._id } >{item.nameService}</option>
           ))
         ) : (
          <p>Loading</p>
         ) }
       </select>
       <label htmlFor="">Jasa Layanan</label><br />

       <select name="profesiId" value={profile.profesiId} onChange={handleChange}>
         {listProfesi.length !== 0 ? (
           listProfesi.map((item, index) => (
              <option key={index} value={item._id } >{item.nameProfesi}</option>
           ))
         ) : (
          <p>Loading</p>
         ) }
       </select>
       <label htmlFor="">Pilihan Profesi</label><br />
